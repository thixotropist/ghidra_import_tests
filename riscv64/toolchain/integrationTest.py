...
generated by gcc-13 and gcc-14 C and C++ compilers, possibly adding some assembly source code to
...
class T3VectorTests(unittest.TestCase):
    ...
    def test01BuildAndImportIntrinsicsDemos(self):
        ...
        for fileName in self.vector_intrinsic_files:
            ...
            build_result = \
                self.bazel.execute(Bazel.VECTOR_PLATFORM, f"gcc_vectorization:{fileName}_vector", operation='build', mode='opt')
            ...
            self.assertEqual(0, build_result.returncode,
                        f"bazel {Bazel.VECTOR_PLATFORM} build of gcc_vectorization/{fileName}_vector failed")

            # import into the Bazel project
            import_result = \
            self.ghidra.import_binary(f"bazel-bin/gcc_vectorization/{fileName}_vector")
            ...
            self.assertEqual(0, import_result.returncode,
                         f"Ghidra import failed for gcc_vectorization/{fileName}_vector ")
