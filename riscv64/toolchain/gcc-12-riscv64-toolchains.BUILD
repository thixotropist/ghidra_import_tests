...
cc_library(
    name = "std_includes",
    hdrs = [
        "sysroot/usr/include/stdc-predef.h",
    ],
)

filegroup(
    name = "riscv64_libexec",
    srcs = glob(["libexec/gcc/riscv64-unknown-linux-gnu/12.2.0/**"]),
)

filegroup(
    name = "riscv64_lib",
    srcs = glob(["lib/gcc/riscv64-unknown-linux-gnu/12/*"])
)

filegroup(
    name = "c++_std_includes",
    srcs = glob(["riscv64-unknown-linux-gnu/include/c++/12.2.0/*"])
)

cc_library(
    name = "c++_iostream_includes",
    hdrs = glob(["riscv64-unknown-linux-gnu/include/c++/12.2.0/*"])
)
