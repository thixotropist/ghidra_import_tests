...
"""
Generate crossplatform binaries for use in Ghidra testing.  Start with riscv64 binaries generated by gcc-12 C and C++
compilers.
...
class Bazel():
    ...
    * `TEST_HOST` run integration tests on the host computer with a CI product emulation platform
    ...
    options = {
        ...
        'output_base': "--output_base=/run/user/%d/bazel" % os.getuid(),
    ...
    TEST_HOST = BASE_COMMAND + f" test {options['distdir']} {options['toolchain_resolution']} {options['bzlmod']}"
    ...
    def execute(self, platform, target, operation='build', mode='dbg'):
        ...
        command = ['bazel',                                 # invoke bazel
                    ...
                    Bazel.options['save_temps'],            #   keeping intermediate .S assembly files
...
class Toolchain():
    ...
    * a riscv64 toolchain matching the deployment instruction set, system root, and base library set.
    * an x86_64 toolchain used by the Continuous Integration test server, ideally with identical versions of gcc, glib, etc.
    * an optional local host toolchain for checking basic C and C++ syntax or generating locally-executed tools
    ...
    * various dependencies, such as a custom RISC-V libstdc++ and an imported Bazel Module like googletest or glog

    The biggest challenges in toolchain debugging tend to involve linking and loading, where `gcc` or `g++` implicitly
    invokes `collect2`, `ld`, `ar`, and various linker scripts.  It is very hard to prove that these implicit dependencies
    are taken from the imported toolchain and not from the host linker environment.  The dynamic libraries (`.so`) needed
...
class Ghidra():
    """
    Collect Ghidra headless analyzer commands
    """
    ...
    def import_binary(self, binaryPath, preScript='', postScript=''):
        command = [Ghidra.GHIDRA_RUN,
                   ...
                   '-overwrite',
                   '-import', os.getcwd() + '/' + binaryPath]
        if (self.scriptPath):
            command = command + ['-scriptpath', os.getcwd() + self.scriptPath]
        if (preScript):
...
class T0ToolchainTest(unittest.TestCase):
    ...
    def test00VerifyToolchainResolution(self):
        ...
        with open('./.bazelrc', 'r') as bazelrcFile:
    ...
    def test01LocalCHelloWorld(self):
        """  local host toolchain build of helloWorld """
        result = self.bazel.execute(Bazel.CI_PLATFORM, Toolchain.REFERENCE_C_PGM, operation='build', mode='dbg')
        self.assertEqual(0, result.returncode,
            'bazel %s build of %s failed' % (Bazel.CI_PLATFORM, Toolchain.REFERENCE_C_PGM))
    ...
    def test03RiscV64Build(self):
        ...
        invoked
        """
        result = self.bazel.execute(Bazel.PRODUCT_PLATFORM, Toolchain.REFERENCE_C_PGM, operation='build', mode='dbg')
        self.assertEqual(0, result.returncode,
            'bazel %s build of %s failed' % (Bazel.PRODUCT_PLATFORM, Toolchain.REFERENCE_C_PGM))

        self.bazel.logger.info("Running: %s", ' '.join(('file', 'bazel-bin/userSpaceSamples/_objs/helloworld/helloworld.pic.o')))
        result = subprocess.run(['file', 'bazel-bin/userSpaceSamples/_objs/helloworld/helloworld.pic.o'],
    ...
    def test04RiscV64AssemblyBuild(self):
        ...
        self.bazel.logger.info("Running: %s", ' '.join(('file', 'bazel-bin/userSpaceSamples/_objs/syntheticRelocations/syntheticRelocations.pic.o')))
        result = subprocess.run(['file', 'bazel-bin/userSpaceSamples/_objs/syntheticRelocations/syntheticRelocations.pic.o'],
            check=True, capture_output=True, encoding='utf8')
        self.assertRegex(result.stdout, 'ELF 64-bit LSB relocatable, UCB RISC-V',
            f'//platforms:{Bazel.PRODUCT_PLATFORM} compilation generated an unexpected object file format' )
...
class T1ImportTests(unittest.TestCase):
    ...
    def test01HelloWorld(self):
        ...
        self.assertEqual(0, result.returncode,
            'bazel %s build of %s failed' % (Bazel.PRODUCT_PLATFORM, 'helloworld'))
        result = self.ghidra.import_binary('bazel-bin/userSpaceSamples/helloworld')
        self.assertEqual(0, result.returncode,
                         'Ghidra imported userSpaceSamples:helloworld executable built for riscv64')
        result = self.ghidra.import_binary('bazel-bin/userSpaceSamples/_objs/helloworld/helloworld.pic.o')
        self.assertEqual(0, result.returncode,
